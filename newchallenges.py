...
app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://localhost/moviedirdb"
...
def result():
    ...
    if form.validate_on_submit():
        ...
        d = Director.query.filter_by(directorName=director_name).first() #like a select sql query

        if d:
            print("Director exists", d.directorId)

        else:
            d = Director(directorName = director_name) #create director if it doesn't exist
            db.session.add(d)
            db.session.commit()
